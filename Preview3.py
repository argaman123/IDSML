...
if not path.exists("ML"):
    dataHandler = DatasetHandler(filename="150218IDS.csv", numrows=1000000, trainAmount=1, normalText="Benign",resultsColumn="Label",
...
testingCSV = Dataset(filename="2020-08-11_Flow.csv", resultsColumn="Label", dataTypes=dTypes, droppedColumns=["Flow ID","Src IP","Src Port","Dst IP"])

for row in testingCSV:
    print(f"{mlloaded.isNormal(row)}: {mlloaded.dataHandler.prepareData(row)}")
